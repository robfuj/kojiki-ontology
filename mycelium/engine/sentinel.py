...
try:
    ...
    class MockPrivateKey:
        ...
        def private_bytes(self, encoding, format, encryption_algorithm):
            return b"mock_private_pem"
    
    class MockPublicKey:
        def verify(self, signature, data):
            pass
        def public_bytes(self, encoding, format):
            return b"mock_public_pem"
...
def verify_signature(signature: str, data: str, public_key_str: str) -> bool:
    ...
    Args:
        signature: Signature in format "ed25519:<base64>"
        data: The data that was signed
        public_key_str: Public key in format "ed25519:<base64_pem>"
    ...
    if HAS_CRYPTOGRAPHY:
        ...
        # Mock verification - accept mock signatures for mock keys
        return signature.startswith("ed25519:") and public_key_str.startswith("ed25519:")
...
class ProvenanceToken:
    ...
    payload_ref: str
    payload_hash: str
    entry_type: str  # "signal", "edge_history", "gate_evidence", "registration_approved", "registration_rejected", "key_repaired", "key_rotated"
    recorded_at: str
...
class KeyManager:
    ...
    def __init__(self, keys_dir: Optional[str] = None):
        # Default to user home directory outside project tree
        if keys_dir is None:
            keys_dir = str(Path.home() / ".kojiki" / "sentinel" / "keys")
        self.keys_dir = Path(keys_dir)
        self.keys_dir.mkdir(parents=True, exist_ok=True)
        self.private_keys: Dict[str, Ed25519PrivateKey] = {}
        self.public_keys: Dict[str, Ed25519PublicKey] = {}

    def generate_keypair(self, node_id: str) -> Tuple[Ed25519PrivateKey, str]:
            """Generate and store keypair for a node. Returns (private_key, public_key_pem)."""
            ...
            public_key_registry_format = f"ed25519:{public_key_b64}"
        
            self.private_keys[node_id] = private_key
            self.public_keys[node_id] = public_key
        
            # Save private key to file (node runtime)
            # Sanitize node_id for filesystem safety (spaces, special chars)
            safe_node_id = re.sub(r'[^a-zA-Z0-9._-]', '_', node_id)
            key_file = self.keys_dir / f"{safe_node_id}_private.pem"
            key_file.write_bytes(private_pem)
        
            return private_key, public_key_registry_format
    ...
    def load_private_key(self, node_id: str) -> Optional[Ed25519PrivateKey]:
        ...
        # Sanitize node_id for filesystem safety (spaces, special chars)
        safe_node_id = re.sub(r'[^a-zA-Z0-9._-]', '_', node_id)
        # Also try with dots replaced by underscores (legacy format)
        legacy_safe_node_id = safe_node_id.replace('.', '_')
        
        # Try new format first
        key_file = self.keys_dir / f"{safe_node_id}_private.pem"
        if not key_file.exists() and legacy_safe_node_id != safe_node_id:
            # Try legacy format (dots -> underscores)
            key_file = self.keys_dir / f"{legacy_safe_node_id}_private.pem"
        ...
        if key_file.exists():
            ...
            if HAS_CRYPTOGRAPHY:
                try:
                    ...
                    # Corrupted/malformed PEM - regenerate
                    pass
            else:
                # Mock - return a mock private key
                self.private_keys[node_id] = Ed25519PrivateKey.generate()
                return self.private_keys[node_id]
        
        # Only auto-generate if explicitly requested via generate_keypair
        # NOT auto-generating for unknown signers - that's a security risk
        return None
    ...
    def verify(self, node_id: str, data: str, signature: str) -> bool:
        ...
        return verify_signature(signature, data, public_key_str)

    def rotate_key(self, node_id: str, reason: str = "key_rotation") -> Tuple[str, str]:
        """
        Rotate keypair for a node.
        
        Args:
            node_id: Node identifier
            reason: Reason for rotation (audit log)
            
        Returns:
            Tuple of (old_public_key, new_public_key) in registry format
        """
        # Get old public key for audit trail
        old_public_key = self.get_public_key(node_id)
        
        # Generate new keypair
        new_private_key, new_public_key = self.generate_keypair(node_id)
        
        # Log the rotation event (would be written by SentinelEngine)
        # This is the new key that will be used going forward
...
class SentinelLog:
    ...
    def __init__(self, log_path: str, entry_type: str, key_manager: KeyManager):
        ...
        self.entry_type = entry_type  # "signal", "edge_history", "gate_evidence", "node_lifecycle"
    ...
    def write(self, signer: str, payload_ref: str, payload: Any) -> ProvenanceToken:
        ...
        Verifies the token before committing; raises ValueError if verification fails.
        """
        ...
        # Verify the token before committing (fail-closed)
        if not self.verify_token(token):
            raise ValueError(f"Generated token failed verification: {entry_id}")
    ...
    def write_node_lifecycle_event(self, event_type: str, node_id: str, actor: str, detail: Any) -> ProvenanceToken:
        """
        Write a node lifecycle event (registration_approved, registration_rejected, key_repaired, key_rotated).
        Uses a fixed system signer ("registry") for the provenance token; actor is recorded in payload.
        """
        payload = {
        ...
        payload_ref = f"node_lifecycle:{node_id}:{event_type}"
        # Use fixed system signer for audit log integrity
        return self.write("registry", payload_ref, payload)
...
class SentinelEngine:
    ...
    def __init__(self, base_path: str = "mycelium", key_manager: Optional[KeyManager] = None):
        self.base_path = Path(base_path)
        self.key_manager = key_manager or KeyManager(str(self.base_path / "sentinel/keys"))
        
        # Ensure registry keypair exists for audit log signing
        self._ensure_registry_key()
        
        # Four wrapped logs (added node_lifecycle)
        ...
        self.node_lifecycle_log = SentinelLog(
            str(self.base_path / "log/node_lifecycle.jsonl"),
            "node_lifecycle",
            self.key_manager
        )
    
    def _ensure_registry_key(self):
        """Ensure the 'registry' system keypair exists for audit log signing."""
        if "registry" not in self.key_manager.private_keys:
            try:
                self.key_manager.load_private_key("registry")
            except:
                pass
            if "registry" not in self.key_manager.private_keys:
                self.key_manager.generate_keypair("registry")
    ...
    def write_gate_evidence(self, signer: str, exp_ref: str, experience: Dict[str, Any]) -> ProvenanceToken:
        ...
        The signer is the actual node that produced the experience (e.g., the primary agent).
        The experience ref is recorded in payload_ref.
        """
        return self.gate_evidence_log.write(signer, exp_ref, experience)
    
    def write_node_lifecycle_event(self, event_type: str, node_id: str, actor: str, detail: Any) -> ProvenanceToken:
        """
        Write a node lifecycle event to the node_lifecycle log.

        event_type: "registration_approved", "registration_rejected", "key_repaired", "key_rotated"
        """
        return self.node_lifecycle_log.write_node_lifecycle_event(event_type, node_id, actor, detail)
    ...
    def verify_experience_chain(self, exp_ref: str, registry) -> bool:
        """
        Verify the SENTINEL chain for a specific experience reference.
        
        Checks if there's at least one valid provenance token for this experience
        with an active (non-revoked) signer key.
        """
        tokens = self.gate_evidence_log.get_tokens()
        for token in tokens:
            if token.payload_ref == exp_ref:
    ...
    def verify_all_chains(self) -> Dict[str, Tuple[bool, List[str]]]:
        ...
        return {
            ...
            "node_lifecycle": self.node_lifecycle_log.verify_chain()
        }
...
def main():
    ...
    if cmd == "gen-key":
        ...
        if log_type == "signal":
            ...
            # Expected payload: {"event_type": "...", "node_id": "...", "actor": "...", "detail": ...}
            event_type = payload.get("event_type")
            target_node_id = payload.get("node_id")
            actor = payload.get("actor")
            detail = payload.get("detail")
            token = engine.write_node_lifecycle_event(event_type, target_node_id, actor, detail)
        else:
    ...
    elif cmd == "verify-signature":
        if len(sys.argv) < 5:
            print("Usage: python sentinel.py verify-signature <public_key> <signature> <data>")
            return
        public_key = sys.argv[2]
        signature = sys.argv[3]
        data = sys.argv[4]
        ok = verify_signature(signature, data, public_key)
