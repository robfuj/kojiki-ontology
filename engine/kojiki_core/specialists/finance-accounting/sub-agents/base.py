#!/usr/bin/env python3
"""
Base class for finance sub-specialists.
"""
...
class FinanceSubSpecialist(Specialist):
    """Base class for finance sub-specialists."""
    ...
    @property
    def name(self) -> str:
        return f"finance-accounting.{self.SUB_AGENT_NAME}"
    
    @property
    def agent_prefix(self) -> str:
        return f"finance.{self.SUB_AGENT_NAME}"
    
    @property
    def decision_rights_node(self) -> str:
        return f"Finance.{self.SUB_AGENT_NAME.replace('-', ' ').title().replace(' ', '')}"
