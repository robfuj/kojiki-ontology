#!/usr/bin/env python3
"""
Base class for sales sub-specialists.
"""
...
class SalesSubSpecialist(Specialist):
    """Base class for sales sub-specialists."""
    ...
    @property
    def name(self) -> str:
        return f"sales-outbound.{self.SUB_AGENT_NAME}"
    
    @property
    def agent_prefix(self) -> str:
        return f"sales.{self.SUB_AGENT_NAME}"
    
    @property
    def decision_rights_node(self) -> str:
        return f"Sales.{self.SUB_AGENT_NAME.replace('-', ' ').title().replace(' ', '')}"
