#!/usr/bin/env python3
"""
Base class for engineering sub-specialists.
"""
...
class EngineeringSubSpecialist(Specialist):
    """Base class for engineering sub-specialists."""
    ...
    @property
    def name(self) -> str:
        return f"engineering-platform.{self.SUB_AGENT_NAME}"
    
    @property
    def agent_prefix(self) -> str:
        return f"engineering.{self.SUB_AGENT_NAME}"
    
    @property
    def decision_rights_node(self) -> str:
        return f"Engineering.{self.SUB_AGENT_NAME.replace('-', ' ').title().replace(' ', '')}"
