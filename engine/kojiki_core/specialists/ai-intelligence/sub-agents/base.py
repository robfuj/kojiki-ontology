#!/usr/bin/env python3
"""
Base class for AI Intelligence sub-specialists.
"""
...
class AISubSpecialist(Specialist):
    """Base class for AI Intelligence sub-specialists."""
    ...
    @property
    def name(self) -> str:
        return f"ai-intelligence.{self.SUB_AGENT_NAME}"
    
    @property
    def agent_prefix(self) -> str:
        return f"ai.{self.SUB_AGENT_NAME}"
    
    @property
    def decision_rights_node(self) -> str:
        return f"AI.{self.SUB_AGENT_NAME.replace('-', ' ').title().replace(' ', '')}"
