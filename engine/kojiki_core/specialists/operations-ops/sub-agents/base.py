#!/usr/bin/env python3
"""
Base class for operations sub-specialists.
"""
...
class OperationsSubSpecialist(Specialist):
    """Base class for operations sub-specialists."""
    ...
    @property
    def name(self) -> str:
        return f"operations-ops.{self.SUB_AGENT_NAME}"
    
    @property
    def agent_prefix(self) -> str:
        return f"operations.{self.SUB_AGENT_NAME}"
    
    @property
    def decision_rights_node(self) -> str:
        return f"Operations.{self.SUB_AGENT_NAME.replace('-', ' ').title().replace(' ', '')}"
