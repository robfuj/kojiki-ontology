#!/usr/bin/env python3
"""
Base class for marketing sub-specialists.
All sub-specialists inherit from this to reduce boilerplate.
"""
...
class MarketingSubSpecialist(Specialist):
    """Base class for marketing sub-specialists."""
    ...
    @property
    def name(self) -> str:
        return f"marketing-brand.{self.SUB_AGENT_NAME}"
    
    @property
    def agent_prefix(self) -> str:
        return f"marketing.{self.SUB_AGENT_NAME}"
    
    @property
    def decision_rights_node(self) -> str:
        return f"Marketing.{self.SUB_AGENT_NAME.replace('-', ' ').title().replace(' ', '')}"
