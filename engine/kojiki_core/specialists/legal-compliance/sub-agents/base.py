#!/usr/bin/env python3
"""
Base class for legal sub-specialists.
"""
...
class LegalSubSpecialist(Specialist):
    """Base class for legal sub-specialists."""
    ...
    @property
    def name(self) -> str:
        return f"legal-compliance.{self.SUB_AGENT_NAME}"
    
    @property
    def agent_prefix(self) -> str:
        return f"legal.{self.SUB_AGENT_NAME}"
    
    @property
    def decision_rights_node(self) -> str:
        return f"Legal.{self.SUB_AGENT_NAME.replace('-', ' ').title().replace(' ', '')}"
