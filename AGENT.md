# AGENT.md — Kojiki Ontology (shared standard)
...
agent harness) at this file and follow the steps. No specific runtime is required.
...
## What this repo is
The **shared standard** every department references. It defines the canonical schemas
(Learning Ledger + Decision Object + Orientation), the learning taxonomy, the
decision-rights model, and the cross-functional handoff standard. Departments do not
redefine these; they import them.

## Orientation for the whole org
When this repo is installed as the organization's root, run the Orientation Protocol
above. The captured `group_id` becomes the registration namespace that every
department and sub-function bot joins via `handoffs/registry.json`.

## SYNAPSIS (the cognitive substrate)
All reasoning runs through **SYNAPSIS** — the bounded transformation engine in
`synapsis/`. Each decision is decomposed into transformations (SOURCE → RECORD →
EVIDENCE → INTERPRETATION → STRATEGY → INTERACTION → OUTPUT → OUTCOME → LEARNING)
so errors are inspectable, not silent. Validate any record with `synapsis/validate.py`.

## How departments use this
A department repo's `AGENT.md` points back here for schemas and the handoff standard.
Install a department (e.g. `03-marketing`) and follow its `AGENT.md` to start.
