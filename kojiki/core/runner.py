...
All stage logic extracted to kojiki/core/stages/
"""
...
SENTINEL_PATH = Path(__file__).parent.parent.parent / "00-kojiki-ontology" / "mycelium" / "engine" / "sentinel.py"
...
# Import registry
POSTGRES_REGISTRY_PATH = Path(__file__).parent.parent.parent / "00-kojiki-ontology" / "mycelium" / "engine" / "postgres_registry.py"
...
REGISTRY_PATH = Path(__file__).parent.parent.parent / "00-kojiki-ontology" / "mycelium" / "engine" / "registry.py"
NodeRegistry = None
...
class PipelineRunner:
    ...
    def __init__(self, specialist: Specialist, dispatch: Dict[str, Any]):
        ...
        if PostgresNodeRegistry:
            ...
            REGISTRY_FILE = Path(__file__).parent.parent.parent / "00-kojiki-ontology" / "mycelium" / "registry" / "nodes.json"
            if REGISTRY_FILE.parent.exists():
                self.registry = NodeRegistry(str(REGISTRY_FILE))
        
        # Schema validator
        self.schema_validator = None
        VALIDATOR_PATH = Path(__file__).parent.parent.parent / "00-kojiki-ontology" / "synapsis" / "schema_validator.py"
...
def main():
    """CLI entry point: python -m kojiki.core.runner <specialist> <dispatch.json>"""
    if len(sys.argv) < 3:
        print("Usage: python -m kojiki.core.runner <specialist_name> <dispatch_json_file>")
        sys.exit(1)
